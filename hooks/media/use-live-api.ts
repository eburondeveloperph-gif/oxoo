      ...
      for (const fc of toolCall.functionCalls) {
        ...
        if (fc.name === 'fetch_google_api') {
           ...
           if (method && method.toUpperCase() !== 'GET') {
               const confirmed = window.confirm(`The assistant wants to perform a ${method} operation to ${url}. Proceed?`);
               if (!confirmed) {
                   responsePayload = { error: 'User cancelled the operation.' };
        ...
        if (fc.name === 'create_keep_note') {
            ...
            const confirmed = window.confirm(`Create Google Keep note "${title}"?`);
            if (!confirmed) {
                functionResponses.push({ id: fc.id, name: fc.name, response: { error: 'User cancelled' } });
                continue;
            }
        ...
        if (fc.name === 'send_chat_message') {
            ...
            const confirmed = window.confirm(`Send Google Chat message to space ${spaceName}?`);
            if (!confirmed) {
                functionResponses.push({ id: fc.id, name: fc.name, response: { error: 'User cancelled' } });
                continue;
            }
        ...
        if (fc.name === 'create_meet_link') {
            ...
            const confirmed = window.confirm(`Create Google Meet link via Calendar for "${summary}"?`);
            if (!confirmed) {
                functionResponses.push({ id: fc.id, name: fc.name, response: { error: 'User cancelled' } });
                continue;
            }
        ...
        if (fc.name === 'list_whatsapp_messages') {
            try {
                const { limit } = fc.args as any;
                responsePayload = await api.listWhatsappMessages(limit);
            } catch (e: any) {
                responsePayload = { error: e.message };
            }
        }
        ...
        if (fc.name === 'send_email') {
           ...
           const confirmed = window.confirm(`Send email to ${recipient} with subject "${subject}"?`);
           if (!confirmed) {
               functionResponses.push({ id: fc.id, name: fc.name, response: { error: 'User cancelled' } });
               continue;
           }
        ...
        if (fc.name === 'create_calendar_event') {
           ...
           const confirmed = window.confirm(`Create calendar event "${summary}" from ${startTime} to ${endTime}?`);
           if (!confirmed) {
               functionResponses.push({ id: fc.id, name: fc.name, response: { error: 'User cancelled' } });
               continue;
           }
        ...
        if (fc.name === 'set_reminder') {
           ...
           const confirmed = window.confirm(`Create task/reminder "${task}" due at ${time}?`);
           if (!confirmed) {
               functionResponses.push({ id: fc.id, name: fc.name, response: { error: 'User cancelled' } });
               continue;
           }
        ...
        if (fc.name === 'calculate') {
         ...
         if (fc.name === 'generate_eburon_document') {
             const { docId, data } = fc.args as any;
             try {
                ...
                const artifact = { title: `Generated ${docId}`, type: "html", content: html };
                const uiState = await import('../../lib/state');
                uiState.useUI.getState().setIsGenerating(true);
                // We DON'T set activeWorkspaceResult immediately here anymore 
                // if we want it to show as a card first. 
                // But let's keep the option open.
                
                // Add a turn for the file card
                useLogStore.getState().addTurn({
                    role: 'agent',
                    text: `I've generated the ${docId} for you.`,
                    isFinal: true,
                    artifact
                });
                ...
                responsePayload = { status: 'Document generated and displayed as a file card to user', docId };
             } catch (e: any) {
                responsePayload = { error: e.message };
             }
         }
        ...
        if (fc.name === 'open_browser_url') {
         ...
         if (fc.name === 'create_html_document' || fc.name === 'create_json_file' || fc.name === 'generate_artifact' || fc.name === 'create_markdown_document' || fc.name === 'create_chart_spec' || fc.name === 'create_project_brief' || fc.name === 'create_checklist') {
           ...
           try {
             ...
             const artifact = { title, type: actualType, content: actualContent, language };
             
             const uiState = await import('../../lib/state');
             uiState.useUI.getState().setIsGenerating(true);
             
             // Add turn with artifact card
             useLogStore.getState().addTurn({
                 role: 'agent',
                 text: `I've created the ${title} artifact for you.`,
                 isFinal: true,
                 artifact
             });
