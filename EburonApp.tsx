...
import { 
  ...
  Plug, Lock, Pencil, Maximize2, FileText, FileCode, FileJson, FileBarChart
} from 'lucide-react';
...
function FileCard({ artifact }: { artifact: any }) {
  const setActiveWorkspaceResult = useUI((state) => state.setActiveWorkspaceResult);
  
  const getIcon = () => {
    switch (artifact.type) {
      case 'html': return <FileText size={24} className="text-blue-400" />;
      case 'markdown': return <FileCode size={24} className="text-emerald-400" />;
      case 'json': return <FileJson size={24} className="text-amber-400" />;
      case 'chart': return <FileBarChart size={24} className="text-rose-400" />;
  ...
  return (
    <div 
      ...
      onClick={() => setActiveWorkspaceResult({ artifact })}
      style={{
        display: 'flex',
        alignItems: 'center',
        gap: '16px',
    ...
    >
      <div className="file-icon-bg" style={{ 
        width: '48px', 
        height: '48px', 
        borderRadius: '10px', 
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: '14px', fontWeight: 600, color: '#fff' }}>{artifact.title}</div>
        <div style={{ fontSize: '12px', color: 'var(--text-muted)', textTransform: 'uppercase', marginTop: '2px' }}>{artifact.type} • 1.2 KB</div>
      </div>
...
export default function EburonApp() {
  ...
  useEffect(() => {
    ...
    setConfig({
      ...
      systemInstruction: {
        parts: [{ text: `${systemPrompt}\n\nUser context:\n${memoryStr}\n\nUser calls themselves: ${userCallName}\nAssistant name: ${personaName}` }]
      },
      tools: allTools
    } as any);
            ...
            <div id="conversation-container">
              ...
              {filteredTurns.map((turn, i) => (
                <div key={i} className={`conversation-message ${turn.role === 'user' ? 'user' : 'ai'}`}>
                    ...
                    {turn.artifact && (
                      <FileCard artifact={turn.artifact} />
                    )}
                </div>
              ))}
            </div>
